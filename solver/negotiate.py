...
from cpsat_route import geometry, write_routes, parse_place, ROWS
import check_routes
...
def route(g, rounds, seed, trace=False):
    ...
    def sp(start, goals, pres):
        ...
        while pq:
            ...
            for w in nb[u]:
                ...
                nd = d + (1 + hist.get(w, 0)) * (1 + pres * occ.get(w, 0)) + noise[w]
    ...
    for it in range(rounds):
        ...
        for n in order:
            for c in {c for p in paths.get(n, {}).values() for c in p}: occ[c] -= 1
            if n == "SH":
                p1 = sp(g["sh_src"][0], snk, pres)
                p2 = sp(g["sh_src"][1], snk | set(p1), pres) if p1 else None
                new = {"SH1": p1, "SH2": p2}
            else:
                s, t = g["nets"][n]; new = {n: sp(s, {t}, pres)}
            if any(p is None for p in new.values()): return dict(rounds=it + 1, fail="unroutable net " + n)
            paths[n] = new
            for c in {c for p in new.values() for c in p}: occ[c] = occ.get(c, 0) + 1
...
def main():
    ...
    for cfg in cfgs:
        ...
        for seed in range(a.seeds):
            t0 = time.time()
            res = route(g, a.rounds, seed)
            rec = dict(tag=tag, cfg=P, seed=seed, rounds=res["rounds"], secs=round(time.time() - t0, 1), at=time.strftime("%F %T"))
            if "paths" in res:
                fn = os.path.join(a.out, "negotiated", f"{tag}_s{seed}.py")
